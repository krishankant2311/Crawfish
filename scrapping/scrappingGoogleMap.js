...
async function getRestaurantDetails(page, link) {
  try {
    ...
    const details = await page.evaluate(() => {
      const name = document.querySelector('h1')?.innerText || '';
      const description =
        document.querySelector('[aria-label^="About"] ~ div div span')?.innerText || '';
...
async function scrapeGoogleMaps(lat, lng, address) {
 ...
 const browser = await puppeteer.launch({
  ...
  for (const r of basicResults) {
    ...
    const existing = await Restaurant.findOne({
      $or: [
        { restaurantName: new RegExp(`^${r.title}$`, 'i') },
        { fullAddress: new RegExp(`^${r.address}$`, 'i') },
        { website: r.link },
      ],
    });
    ...
    const restaurant = new Restaurant({
      restaurantName: r.title,
      fullAddress: details.fullAddress || r.address,
      address: details.fullAddress || r.address,
      city: details.city,
      country: details.country,
      location: {
